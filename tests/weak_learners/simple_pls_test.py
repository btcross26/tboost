...
def test_fit_max_vars_1(threshold):
    ...
    assert torch.all(model.coef_[:7] == 0.0)
    ...
    Xs = torch.hstack([torch.ones(X.shape[0]).reshape(-1, 1), X[:, [7]]])
    ls_coef = np.linalg.lstsq(Xs.numpy(), y.numpy(), rcond=None)[0].astype(np.float64)
    ls_coef = torch.from_numpy(ls_coef)
    torch.testing.assert_close(
        tensor([model.intercept_, model.coef_[7]], dtype=torch.float64),
        ls_coef,
        atol=0.0,
        rtol=1e-6,
    )
...
def test_fit_multiple_vars(num_vars):
    ...
    assert (model.coef_ != 0.0).sum().item() == num_vars, (
        (model.coef_ != 0.0).sum().item()
    )
...
def test_predict_method():
    ...
    obj = MagicMock()
    obj.coef_ = tensor([0.0, 1.0, 2.0, 0.0, 3.0, -4.0, 0.0, -5.0], dtype=torch.float64)
    obj.intercept_ = 2.0
...
def test_mask_coefs_max_vars(num_vars):
    ...
    if num_vars == 2:
        assert torch.all(model.coef_[[7, 0]] != 0.0)
        assert torch.all(model.coef_[[1, 2, 3, 4, 5, 6]] == 0.0)

    # THEN a 3-var model should only have non-zero coefficient columns 7, 0, and 3
    if num_vars == 3:
        assert torch.all(model.coef_[[7, 0, 3]] != 0.0)
        assert torch.all(model.coef_[[1, 2, 4, 5, 6]] == 0.0)
...
def test_mask_coefs_filter_threshold(threshold):
    ...
    if threshold <= 0.0:
        assert torch.all(model.coef_ != 0.0)
